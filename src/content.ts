...
function zipObjectArrays<T extends Record<string, any[]>>(obj: T): Array<{ [K in keyof T]: T[K][number] }> {
  const keys = Object.keys(obj) as (keyof T)[];
  const length = Math.min(...keys.map(k => obj[k].length));

  return Array.from({ length }, (_, i) =>
    Object.fromEntries(keys.map(k => [k, obj[k][i]])) as { [K in keyof T]: T[K][number] }
  );
}
