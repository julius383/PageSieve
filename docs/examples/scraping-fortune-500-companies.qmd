...
[Download Config](./50pros.com__fortune500-executive_team.json)
...
```json
{
  ...
  "selectors": [
    {
      "id": "g_E60uqc",
      "container": "td[colspan=\"9\"] .grid > div", // <1>
      "fields": [
        {
          "id": "f_8gaIvs",
          "name": "Role",
        ...
        },
        {
          "id": "f_qn54EZ",
          "name": "Company Name",
          "selector": "../../../../preceding-sibling::tr[1]/td[2]/a/text()", // <2>
          "type": "single",
          "extract": "text",
          "required": false
        }
...
1.  We define a container selector (line 14) in this case because the information for each executive team member is encapsulated in a recognizable DOM element. The rest of the fields are can be obtained using the interactive picker or directly inspecting the relevant parts of the page. 
2.  The "Company Name" (lines 32-35) field requires special attention. Since we have defined a container element, we use an XPath selector to go up in the document tree.

:::{.callout-note}
We use a relative axis to go up in the tree before using `preceding-sibling`
which selects siblings of the current node within the same parent.
In our particular case we find the first `tr` element that's before the current node
